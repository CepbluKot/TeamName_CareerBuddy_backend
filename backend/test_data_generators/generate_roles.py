...
def generate_roles():
    ...
    for role in all_roles:
        role_db_record = Roles(
            name=role, admin_page=False, feedback_page=True, career_goal_page=True
        )

        if role == "Human Resources":
            role_db_record = Roles(
                name=role, admin_page=True, feedback_page=True, career_goal_page=True
            )

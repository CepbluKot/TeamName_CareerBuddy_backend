...
def generate_feedback_templates(creator_employee_id: int):
    ...
    template1 = FeedbackTemplates(
        name=fake.text(20),
        content=tempalte_1_content,
        created_by_employee_id=creator_employee_id,
    )
    template2 = FeedbackTemplates(
        name=fake.text(20),
        content=tempalte_2_content,
        created_by_employee_id=creator_employee_id,
    )
...
def generate_feedback():
    ...
    hr_employees = db.session.execute(
        ...
        .filter(Departments.name == "Human Resources")
    ).all()
    not_hr_employees = db.session.execute(
        select(Employees.id, Departments.name)
        .filter(Employees.role_id == Departments.id)
        .filter(Departments.name != "Human Resources")
    ).all()
